...
static bool I2C_WritePacket(uint8_t targetAddr, uint8_t *pBuffer, uint8_t length) {
    ...
    // 【核心修复】清除/重置 FIFO 及 Trigger 中断状态
...
bool MPU6500_Init(void) {
    uint8_t who_am_i = 0;
    UART_Puts(&g_uart0, "MPU6500 6-Axis Init...\r\n");

    DL_Common_delayCycles(3200000);

    I2C_WriteReg(MPU6500_PWR_MGMT_1, 0x80);
    DL_Common_delayCycles(3200000);

    if (!I2C_WriteReg(MPU6500_PWR_MGMT_1, 0x01)) return false;
    if (!I2C_WriteReg(MPU6500_SMPLRT_DIV, 0x07)) return false;
    if (!I2C_WriteReg(MPU6500_CONFIG, 0x03)) return false;
    if (!I2C_WriteReg(MPU6500_GYRO_CONFIG, 0x18)) return false;

    if (!I2C_WriteReg(MPU6500_ACCEL_CONFIG, 0x00)) {
        UART_Puts(&g_uart0, "Err: ACCEL_CONFIG\r\n");
        return false;
    }

    if (!I2C_ReadRegs(MPU6500_WHO_AM_I, &who_am_i, 1)) return false;
    if (who_am_i != 0x70 && who_am_i != 0x68) return false;

    UART_Puts(&g_uart0, "MPU6500 Communication OK\r\n");
    return true;
}
    ...
    switch (DL_I2C_getPendingInterrupt(I2C_GYRO_INST)) {
        ...
        case DL_I2C_IIDX_CONTROLLER_TXFIFO_TRIGGER:
            if (gTxCount < gTxLen) {
                ...
                /* 【核心修复】FIFO 数据填充完毕后强行关掉 TXFIFO 中断 */
                DL_I2C_disableInterrupt(
                    I2C_GYRO_INST, DL_I2C_INTERRUPT_CONTROLLER_TXFIFO_TRIGGER);
            }
            break;
        ...
        case DL_I2C_IIDX_CONTROLLER_STOP:
            /* 【核心修复】响应由 SysConfig 默认使能的 STOP 中断 */
        ...
        case DL_I2C_IIDX_CONTROLLER_NACK:
            /* 【核心修复】移除死板限制，只要不是 IDLE 状态，发生 NACK 均切为 ERROR */
